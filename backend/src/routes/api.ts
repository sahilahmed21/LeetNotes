...
router.post("/generate-notes/:problemId", async (req: AuthRequest, res) => {
    ...
    let problemData: any; // Use 'any' for now, or define a more precise type including submissions structure
    try {
        ...
        // ... (keep problem fetch error handling) ...
        if (problemError) {
        ...
        // Find the most recent accepted submission, if any
        const acceptedSubmissions = (problemData.submissions || [])
            .filter((s: any) => s.status === 'Accepted')
            .sort((a: any, b: any) => parseInt(b.timestamp || '0', 10) - parseInt(a.timestamp || '0', 10)); // Sort descending by timestamp
        const latestAcceptedSubmission = acceptedSubmissions.length > 0 ? acceptedSubmissions[0] : null;

        // Prepare a summary of recent attempts (optional, provides context)
        const submissionSummary = (problemData.submissions || []).slice(0, 3).map((s: any) => ({
            status: s.status,
            runtime: s.runtime,
            memory: s.memory,
            language: s.language,
            // timestamp: s.timestamp // Optional: include if useful for AI
        }));
...
`;
        // console.log("Prompt length:", prompt.length); // Optional for debugging
        // --- End Step 3 ---
        ...
        // Step 5: Parse Gemini Response (Keep as is, including safety check)
        let generatedText = "";
        try {
        ...
        try {
            // ... (Keep existing parsing logic using sectionMap) ...
            ...
            if (!sections.question) sections.question = problemData.description;
        ...
        // Step 7: Store or Update Note in Supabase (Keep as is, including detailed logging/error handling)
        console.log("Attempting to Store/Update note in Supabase...");
        try {
            // ... (Keep existing select/insert/update logic with its try/catch) ...
            ...
            if (existingNote) {
                ...
                const updatePayload = { ...notePayloadBase, updated_at: new Date().toISOString() }; // Verify if updated_at exists/is needed
                ...
                const insertPayload = { ...notePayloadBase }; // Verify if updated_at needed here
            ...
            // Step 8: Send successful response (Keep as is)
    ...
    } catch (err: any) { // Outer catch (Keep as is)
